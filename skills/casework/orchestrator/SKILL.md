...
1. **Intake.** When the user sends a transcript, the inbound message carries a marker line `[Attachment saved locally: <path>]` (the gateway injects it). Read that exact file path, copy its contents into `~/.hermes/wiki/raw/transcripts/<descriptive-name>.md`, then apply `templates/intake.md` to extract structured fields and reconcile against `sor.get_applicant`/`get_case`. If there is no such marker (no file attached), ask the user to paste the transcript text and proceed with that.
